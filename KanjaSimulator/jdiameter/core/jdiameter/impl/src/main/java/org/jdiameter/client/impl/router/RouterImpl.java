...
public class RouterImpl implements IRouter {
  ...
  public IPeer getPeer(IMessage message, IPeerTable manager) throws RouteException, AvpDataException {
    ...
    if (c != null && c.hasValidConnection()) {
      ...
      // Balancing            IPeer peer = null;      for(IPeer host : availablePeers) {    	  if(host.getUri().getPort() == message.getDestPort())    		  peer = host;      }      if(peer == null)
        ...
        switch (redirectUsage) {
          ...
          case ALL_USER:
           ...
           /* if (userNameAvp == null) {
            ...
            primaryKey = "SWIT_AutoTeam";//userNameAvp.getUTF8String();
            break;
        }
        //
   ...
   * @param destHost
   */
  private void updateRoute(IRequest request, String red_destHost) {
	String destHost = red_destHost.split("//")[1].split(";")[0];	request.getSessionId();	request.setDestHost(destHost);	
    request.getAvps().removeAvp(Avp.DESTINATION_HOST);
    request.getAvps().addAvp(Avp.DESTINATION_HOST, destHost.split(":")[0], true, false,  true);
  }
  ...
  public boolean updateRoute(IRequest message) throws RouteException, AvpDataException {
    ...
    String userName = "SWIT_AutoTeam";//null;
    // get Session id
    String sessionId = message.getSessionId();
    //
