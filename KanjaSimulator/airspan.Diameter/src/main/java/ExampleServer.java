...
public class ExampleServer implements NetworkReqListener {
	...
	private static final int AAA = 265;
	private static final int AVP_IP_CAN_Type = 1027;
	private static final int AVP_RAT_TYPE = 1032;
	
	private static final long vendorID = 66666;
	private static final long applicationID = 16777236;
	private ApplicationId App3gppRX = ApplicationId.createByAuthAppId(applicationID);;
	private ApplicationId App3gppGX = ApplicationId.createByAuthAppId(16777238);;
	...
	private void initStack() {
		...
		try {
			...
			network.addNetworkReqListener(this, new ApplicationId[] {this.App3gppGX , this.App3gppRX});
		} catch (Exception e) {
			e.printStackTrace();
			if (this.stack != null) {
				this.stack.destroy();
	...
	*/
	...
	public Answer processRequest(Request request) {
		...
		if (request.getCommandCode() == AAA) {
			Answer answer = createAAA(request);
	...
	private Answer createAAA(Request r) {
		Answer answer = r.createAnswer(2001);
		AvpSet answerAvps = answer.getAvps();	
		IMetaData tempMeta = (IMetaData) stack.getMetaData();
		answerAvps.addAvp(Avp.ORIGIN_HOST, stack.getMetaData().getLocalPeer().getUri().getFQDN(), true, false, true);
		answerAvps.addAvp(Avp.ORIGIN_REALM, stack.getMetaData().getLocalPeer().getRealmName(), true, false, true);

		answerAvps.addAvp(AVP_IP_CAN_Type,5,10415,true,false);
		answerAvps.addAvp(AVP_RAT_TYPE,1004,10415,true,false);
		answerAvps.addAvp(Avp.ORIGIN_STATE_ID,tempMeta.getLocalHostStateId(), true, false, true);

		AvpSet SupportedFeatures = answerAvps.addGroupedAvp(Avp.SUPPORTED_FEATURES,10415,true,false);
		SupportedFeatures.addAvp(Avp.VENDOR_ID,10415);
		SupportedFeatures.addAvp(Avp.FEATURE_LIST_ID,1,10415,true,false);
		SupportedFeatures.addAvp(Avp.FEATURE_LIST,18,10415,true,false);
		
		answerAvps.removeAvp(Avp.DESTINATION_REALM);
		answerAvps.removeAvp(Avp.SUBSCRIPTION_ID);
